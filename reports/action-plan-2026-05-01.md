...
- [x] **FOSS F-06 (🟢) — `@asteasolutions/zod-to-openapi`** generates an OpenAPI 3.1 spec from the existing Zod schemas (executed 2026-05-04). New [src/lib/server/openapi/spec.ts](../src/lib/server/openapi/spec.ts) calls `extendZodWithOpenApi(z)` once at module load, then registers the four Zod schemas from [validators.ts](../src/lib/server/content/validators.ts) (`projectCreateSchema`, `projectPatchSchema`, `goalCreateSchema`, `goalPatchSchema`) plus the response envelopes / history / me / error shapes that aren't currently expressed as Zod. Imported schemas are cloned via `.clone()` before being passed to `registry.register()` because Zod 4 binds prototype methods (including the patched `.openapi()`) onto each instance at construction time — schemas built before the extension call lack the method, but cloning recreates them post-patch. New [src/routes/api/openapi.json/+server.ts](../src/routes/api/openapi.json/+server.ts) caches the generated document in module scope and serves it with `cache-control: public, max-age=300`. Public reads (`GET /api/{projects,goals}`, history) are unsecured; writes are tagged with two `apiKey` security schemes (`x-editor-token` + `cf-access-jwt-assertion`) reflecting the auth pathways in [editor.ts](../src/lib/server/auth/editor.ts). Verification: `npm run typecheck` 587 files / 0 errors, `npm test` **88 / 88 pass** (6 new in [spec.test.ts](../src/lib/server/openapi/spec.test.ts) covering document version, every CRUD path, security-tagging contract, schema component reuse, and the S-11 413 contract on JSON-body writes), `npm run lint` 0 errors / 109 warnings.
- [x] **Codebase F-11 — Add a real `compilerOptions` block** to [tsconfig.json](../tsconfig.json) ✅ (executed 2026-05-04). Added a project-level `compilerOptions` block enabling **`noUncheckedIndexedAccess`** as the chosen strictness ratchet. This is the highest-value runtime-safety flag: it forces `arr[i]` / `record[key]` reads to be typed as `T | undefined`, which surfaces real "what if the index is past the end" bugs rather than the cosmetic-feeling sites `exactOptionalPropertyTypes` tends to flag. Held back the more aggressive `exactOptionalPropertyTypes` / `noPropertyAccessFromIndexSignature` / etc. for a future ratchet so the team can land them deliberately. The flag surfaced 9 errors across 4 files, all the same shape — array index access after a `length`-based guard the compiler can't follow. Fixes: [src/lib/map/addMetroCountyLayers.ts:108-110](../src/lib/map/addMetroCountyLayers.ts#L108) replaced `if (!features.length) return; const f = features[0];` with the inverted `const f = features[0]; if (!f) return;` (TS narrows on the second form), [src/lib/server/content/validators.ts:111-118](../src/lib/server/content/validators.ts#L111) likewise rewrote `zodErrorToMessage` to read `firstIssue` *then* check, [src/components/svelte/MethodologyModal.svelte:44-49](../src/components/svelte/MethodologyModal.svelte#L44) moved the focus-trap empty-list bail-out below the index reads with `if (!first || !last)`, and [src/lib/server/openapi/spec.test.ts:10](../src/lib/server/openapi/spec.test.ts#L10) tightened `doc.servers?.[0].url` → `doc.servers?.[0]?.url`. Verification: `npm run typecheck` **587 files / 0 errors / 0 warnings**, `npm test` **88 / 88 pass** (unchanged), `npm run lint` 0 errors / **109 warnings** (unchanged baseline).
...
- [x] **UI/UX U-13 — Committed to dark-only explicitly** (executed 2026-05-04). Added `color-scheme: dark` to `:root` in [global.css:11](../src/styles/global.css#L11) and `<meta name="color-scheme" content="dark">` in [src/app.html](../src/app.html). The two-place declaration covers initial paint (meta tag, before CSS parses) *and* CSS-level inheritance, so the UA renders scrollbars, native form controls, and autofill highlights in dark mode for every visitor regardless of OS preference. No light-mode token override added — that's a real design lift (22-token palette + status-badge contrast re-verification of the Phase 4 U-01 work) and nothing in the audit indicated demand. Decision recorded in the new "Color scheme: dark-only" section of [docs/design-tokens.md](../docs/design-tokens.md).
- [x] **Codebase F-10 — Decide policy on Cloudflare Access AUD/team-domain in [wrangler.jsonc](../wrangler.jsonc)** ✅ (executed 2026-05-04). Decision: **keep as `vars`** (committed). Both values are public JWT validation parameters — `CF_ACCESS_TEAM_DOMAIN` is the user-visible `*.cloudflareaccess.com` login host, and `CF_ACCESS_AUD` is the application audience tag checked against the `aud` claim. Neither can forge a token: signing keys live with Cloudflare and are fetched from the JWKS endpoint at runtime. Documented inline with a JSONC comment above the top-level `vars` block in [wrangler.jsonc](../wrangler.jsonc) plus a back-reference above the staging `vars` block.
- [x] **Security S-09 — Per-tester token actor logging.** ✅ Closed as superseded (verified 2026-05-04). S-02's environment gate shipped at [editor.ts:242](../src/lib/server/auth/editor.ts#L242) — the token-mode path now only fires when `isTestEnvironment(event)` returns true (`TEST_HOSTNAMES` ∈ `{localhost, 127.0.0.1, 0.0.0.0, ::1}`, or absent `cf-ray` for `wrangler dev`). Per [security-2026-05-01.md:294](security-2026-05-01.md#L294) ("If S-02 is fixed and the token-mode is genuinely test-only, this is fine") the per-tester actor schema is no longer needed: shared `editor-token-user` `content_history` rows can only originate from local/CI runs, where audit-trail disambiguation isn't load-bearing.
